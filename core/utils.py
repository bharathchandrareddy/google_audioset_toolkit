...
#             ]
#             youtube_dl_output = subprocess.check_output(youtube_dl_command).decode().strip()
...
#                 "-i", youtube_dl_output,
#                 "-ar", str(DEFAULT_FS),
#                 os.path.join(dst_dir, f"{row[0]}_{row[1]}.wav")
#             ]
...
def download(class_name, args):
    ...
    if not os.path.isdir(dst_dir):
        os.makedirs(dst_dir)
        print("dst_dir: " + dst_dir)
    #new_csv_file= remove_duplicates(new_csv, output_csv)
    try:
        with open(new_csv) as dataset:
            reader = csv.reader(dataset)
            ...
            for row in reader:
                ...
                output_file = os.path.join(dst_dir, f"{youtube_id}_{row[1]}.wav")
                ...
                youtube_dl_command = ["yt-dlp", "-f", "bestaudio", "-g", "https://www.youtube.com/watch?v=" + youtube_id]

                try:
                    # Execute youtube-dl command
                    youtube_dl_output = subprocess.check_output(youtube_dl_command).decode().strip()
                except subprocess.CalledProcessError as e:
                    print(f'Error occurred while fetching {youtube_id}: {e}')
                    continue  # Skip to the next iteration if youtube-dl fails

                ffmpeg_command = ["ffmpeg", "-ss", str(row[1]), "-t", "10", "-i", youtube_dl_output, "-ar", str(DEFAULT_FS), "--", output_file]
                subprocess.run(ffmpeg_command)
...
if __name__ == "__main__":
    download(class_name, args)  # Assuming 'args' is defined elsewhere in your script


#this function is to delete duplicate rows from the csv file which is created for the individual class
import csv
import hashlib

def remove_duplicates(input_file, output_file):
    seen_hashes = set()  # Keep track of hashes of rows seen before
    ...
    with open(input_file, 'r', newline='') as infile, \
        ...
        for row in reader:
            row_hash = hashlib.md5(str(row).encode()).hexdigest()  # Compute hash of the row
